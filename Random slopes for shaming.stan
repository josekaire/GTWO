...
transformed parameters{
    vector[2] v_alpha_idbS_id[N_id];
    vector[2] Mu_00;
    cov_matrix[2] SRS_sigma_idRho;
    for ( j in 1:N_id ) {
        v_alpha_idbS_id[j,1] = alpha_id[j];
        v_alpha_idbS_id[j,2] = bS_id[j];
    }
    for ( j in 1:2 ) {
        Mu_00[1] = 0;
        Mu_00[2] = 0;
...
model{
    ...
    bGDP~ normal(0, .5);    
    alpha ~ normal( 0 , 0.25 );
    v_alpha_idbS_id ~ multi_normal( Mu_00 , SRS_sigma_idRho );
    for ( i in 1:N ) {
        mu[i] = alpha + alpha_id[id[i]] + (bS + bS_id[id[i]]) * shaming[i]+
        bP*polity[i]+ bFI*forinv[i]+ bCon*conflict[i] + bPop*poplog[i]+bGDP*gdplog[i]
        + bT * time[i] + bT2 * timesq[i] +   bE * meanshame[i];
    }
